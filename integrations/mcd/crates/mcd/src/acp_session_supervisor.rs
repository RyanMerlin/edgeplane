...
use mcd_acp::SpawnOpts;
use mcd_core::types::AgentSignal;
use mcd_runtimes::claude_agent_acp::AcpSession;
...
async fn run_one_session(
    ...
    let updates_broadcast: ReplayBroadcast<mcd_acp::wire::SessionNotification> =
...
async fn pump_session(
    session: &AcpSession,
    agent_updates: &mut broadcast::Receiver<mcd_acp::wire::SessionNotification>,
    updates_broadcast: &ReplayBroadcast<mcd_acp::wire::SessionNotification>,
    ...
    loop {
        tokio::select! {
            ...
            recv = agent_updates.recv() => {
                match recv {
                    Ok(notif) => {
                        if let mcd_acp::wire::SessionUpdate::UsageUpdate(ref val) = notif.update {
...
mod tests {
    ...
    // (which is the wire_compat integration test in mcd-acp). We
