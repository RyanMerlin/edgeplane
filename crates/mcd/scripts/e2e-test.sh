#!/usr/bin/env bash
# e2e-test.sh — canonical three-runtime demo / acceptance test for mcd v1
#
...
if mc_cmd mesh status 2>&1 | grep -q "running"; then
    ...
    red "daemon not running after mc daemon up"
fi
...
for rt in claude codex gemini; do
    if command -v "$rt" >/dev/null 2>&1; then
        ...
        red "$rt CLI not found — install with: mc daemon runtime install ${rt/claude/claude-code}"
    fi
done
