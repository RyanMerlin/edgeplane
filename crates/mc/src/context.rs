//! Multi-controlplane connection contexts — `~/.mc/contexts.yaml`.
//!
...
//! `~/.mc/sessions/<name>.json` (chmod 600).
//!
//! On first use, the existing `~/.mc/config.json` + `~/.mc/session.json` are
//! treated as the "default" context transparently — no files are rewritten
//! until the user explicitly runs `mc context` commands.
...
pub fn contexts_file_path() -> PathBuf {
    mc_home_dir().join("contexts.yaml")
}

pub fn sessions_dir() -> PathBuf {
    mc_home_dir().join("sessions")
}
