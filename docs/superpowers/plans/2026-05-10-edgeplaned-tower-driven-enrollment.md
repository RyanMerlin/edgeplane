# edgeplaned: Controlplane-Driven Enrollment & State/Config Separation

**Date:** 2026-05-10
**Status:** Design — pending sign-off
**Author:** mc-engineer (Aria) + Merlin
**Supersedes (in part):** Sections of `docs/plans/edgeplaned-persistent-session-architecture.md` describing yaml-based agent enrollment

---

## Why this exists

While wiring up the ACP-native persistent supervisor (Layer 3 of the persistent-session work), we hardened in a yaml shape that duplicates state already owned by `edgeplane-tower`. Reading `EDGEPLANE_PHILOSOPHY.md` after the fact made the gap clear:

> Coordination truth stays in Edgeplane (Postgres). Git is a projection sink, never the authority for mission ownership, approvals, or governance.

The same rule applies to edgeplaned.yaml: **it must not be the authority for mission/agent assignment**. Today it is. Below is what's broken, what the target looks like, and a phased path to get there without throwing away Layers 1–3.

---

## What's broken today

Concrete evidence in the current `DaemonConfig` (`crates/edgeplaned/crates/edgeplaned/src/config.rs`):
...
| "What is edgeplaned doing?" | Acting as a partial agent registry | Per docs: it's a **per-node arm**, like kubelet — claims work, doesn't define work |
| "Reassign aria-work to a new mission?" | Edit yaml on every node, restart daemon | Should be `edgeplane daemon agent reassign` → controlplane UPDATE → daemon rebalances live |
...
Layers 1–3 are not undermined by this. The `edgeplaned-acp` client, the `claude_agent_acp` runtime, and `acp_session_supervisor` are correct building blocks. What's wrong is **what feeds them**: yaml-defined enrollment.
...
| `backend_url` + `token` | `~/.edgeplane/session.json` (edgeplane auth) | already managed by `edgeplane auth` |
...
### New `edgeplaned.yaml` shape

Five lines, max:

```yaml
# ~/.ep/edgeplaned.yaml  (or /etc/edgeplaned/agent.yaml)

# Optional — most fields inherit from `edgeplane auth`
backend_url: http://edgeplane:8008    # optional override
...
### State file: `~/.ep/edgeplaned.state.json` (0600, daemon-managed)
...
```json
{
  ...
  "controlplane_url": "http://edgeplane:8008"
}
```

**Written by:**
- `edgeplane daemon node register --bootstrap-token <jt_…>` (explicit user action), OR
- the daemon's first-run path if it sees a `MCD_BOOTSTRAP_TOKEN` env var

**Read by:** the daemon at every startup. If missing → daemon refuses to start with a clear "run `edgeplane daemon node register` first" message.
...
```
1. Load yaml (5 lines)
2. Resolve auth (edgeplane auth → backend_url + token)
...
2. **MeshAgent record** — `meshagent.capabilities` column (already exists, settable via API). User adjusts via `edgeplane daemon agent capabilities <id> --add foo --remove bar`.
...
The capability dispatcher (`edgeplaned-core::capability_dispatcher`) unions all three when matching against `task.required_capabilities`.

### Mission reassignment flow

```
admin: edgeplane daemon agent reassign aria-work --mission new-research
  ...
  →  edgeplaned on the host running aria-work:
        receive WS event
...
### Phase 4b — edgeplaned: state file + drop node_id/attach_secret from yaml

- [ ] New `state.rs` module: read / write `~/.ep/edgeplaned.state.json` with 0600 perms, schema versioning, atomic rename.
- [ ] `edgeplane daemon node register --bootstrap-token <jt_…>` → POSTs to `/runtime/nodes/register`, captures the response, writes state file.
...
### Phase 4c — edgeplaned: pull agent assignments from controlplane
...
- [ ] `AgentEntry` and `MissionEntry` types removed from `config.rs`. Their replacement is the `MeshAgentRecord` returned by the controlplane (already exists in `edgeplaned-core::types`).
...
- [ ] `edgeplane daemon agent reassign <id> --mission <new>` CLI surface (controlplane endpoint already supports the underlying mutation).
...
- [ ] `edgeplaned-acp::Agent` exposes `InitializeResponse.agentCapabilities` (already captured internally, just needs an accessor).
...
1. **Bootstrap token UX.** `MCD_BOOTSTRAP_TOKEN` env var on first daemon start, OR explicit `edgeplane daemon node register --bootstrap-token <jt_…>` only? I lean explicit — env-var auto-register is too magic. Confirm?
...
5. **What about `attach_bind_addr`?** This stays in yaml today (genuine local infra). Confirm — or should it also live in state, configured via `edgeplane daemon node register --attach-bind 0.0.0.0:8009`?
...
- `edgeplane inject <agent-id>` CLI command (depends on Layer 5)
...
- A user can run `edgeplane daemon node register --bootstrap-token <jt_…>` once on a node.
- They never see `node_id` or `attach_secret` in yaml.
- They run `edgeplane daemon agent enroll --node <node-id> --mission <mission-id> --runtime claude_agent_acp --supervision persistent --profile-path /…` to schedule an agent.
- They run `edgeplane daemon agent reassign <agent-id> --mission <new>` and the daemon rebalances live, no restart.
- `edgeplane daemon.yaml` on a fresh node is 5 lines.
- The Aria fleet (5 profiles, 1 host today) runs as a single `edgeplaned.service` per host with all assignment owned by the controlplane.
