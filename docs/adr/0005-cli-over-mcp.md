# ADR 0005: CLI-First — MCP Is the Agent Runtime Protocol, Not the Operator Interface
...
EdgePlane currently exposes ~65 MCP tools via `edgeplane serve`. Many of these are management and CRUD operations (create domain, list missions, update task, search artifacts). MCP was the path of least resistance to expose tower functionality to AI agents, but it has become the *only* path for many operations — including ones that human operators should be able to run from a shell.
...
Every management and CRUD operation must be reachable via `edgeplane` CLI. MCP may also expose it (agents need it too), but the CLI command comes first — the MCP tool is a projection of the CLI, not the other way around.

Priority surface to close:
...
1. **Write the CLI command first.** If you cannot describe it as a `edgeplane <noun> <verb>` command, the feature surface is not well-designed.
2. **Expose as MCP only if agents need to call it.** Many management operations don't need MCP at all.
...
- Breaking: MCP tools that have CLI equivalents may be deprecated over time; callers should migrate to CLI for management operations
