//! `edgeplane agent attach <agent-id>` — WebSocket client that opens a viewer
//! into a persistent ACP session on a mesh node.
//!
//! The end-to-end path closed by `pump_acp` in edgeplaned is:
//!
//!   edgeplane agent attach
//!     ⇄ WS — controlplane `GET /runtime/nodes/{n}/agents/{a}/attach`
//!     ⇄ HMAC'd dial to edgeplaned `attach_ws`
...
pub async fn run(args: AttachArgs, client: &EdgeplaneClient) -> Result<()> {
    ...
    if !args.remote {
        match crate::agent_ops::describe_local(&args.agent_id).await {
            ...
            Err(e) => {
                ...
                // the operator is targeting a controlplane agent and edgeplaned
...
/// unchanged.
async fn run_acp_attach(args: AttachArgs, client: &EdgeplaneClient) -> Result<()> {
    ...
    let token = client
        .token()
        .ok_or_else(|| anyhow!("no auth token configured — run `edgeplane auth login` first"))?
        .to_string();
...
async fn resolve_node_id(client: &EdgeplaneClient, agent_id: &str) -> Result<String> {
...
mod tests {
    ...
    fn https_to_wss_scheme() {
        let u = build_ws_url("https://edgeplane/", "n", "a").unwrap();
        assert_eq!(u, "wss://edgeplane/runtime/nodes/n/agents/a/attach");
    }

    #[test]
    fn missing_scheme_errors() {
        assert!(build_ws_url("edgeplane", "n", "a").is_err());
    }
}
