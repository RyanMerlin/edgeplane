        ...
        Err(e) => {
            if is_json {
                let err_json = serde_json::json!({
                    "ok": false,
                    "error": e.to_string(),
                    "hint": "Use 'edgeplane capabilities describe <cap>' to see the expected args",
                    "example": format!("edgeplane exec {} --json", args.name),
                });
                eprintln!("{}", serde_json::to_string(&err_json)?);
            } else {
                eprintln!("error: {e}");
                eprintln!(
                    "hint: edgeplane capabilities describe {} for args schema",
                    args.name
                );
            }
            std::process::exit(1);
