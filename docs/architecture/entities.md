...
**Authorization boundary (shipped v0.15.0, Seam 1; node domain-scoping added 2026-07-14, Workstream 2):** every privileged dispatch/ledger/stream action enforces a default-deny `authorized_for_domain` check before acting. Access is granted if the principal is admin, holds the domain in its `domain_scope`, or is listed in `domain.owners`/`domain.contributors`. `domain_scope` is per-principal: for an **agent** it is the single domain in its JWT claim; for a **node** it is resolved dynamically at auth time as the set of domains in which the node currently hosts assigned agents (distinct `meshagent.domain_id` where `runtime_node_id`/`node_id` matches the node), with a short-TTL per-node cache. A node is trusted only within the domains it operates — never globally. Node infra/self paths (register, rotate-token, node-self roster/notify) use separate node-identity gates (`is_authorized_for_node`), not the domain check. Lifecycle mutations additionally require per-task lease ownership (`authz_task_owner`) unless full-trust/admin; `node` remains on the full-trust axis (it legitimately acts on behalf of its hosted agents), but full-trust no longer implies cross-domain reach — domain gates bound it to the node's operating domains.
