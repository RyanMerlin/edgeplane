...
In addition, lifecycle mutations on a `task` row require the caller to present the row's current `claim_lease_id` — the unified completion token, minted at claim time for `kind='claimable'` rows and on first status transition for `kind='assigned'` rows — or match `claimed_by_agent_id` (claimable) / `owner` (assigned) via `authz_task_owner`, unless full-trust or admin. A compromised agent is therefore bounded to its own tasks within its domain.
...
- **Claim loop**: polls for `task` rows with `kind='claimable'` and `status='ready'` whose `claim_policy.target_profile` matches a profile supervised on this node. For each match: enrolls an ephemeral `MeshAgent` under the parent `Agent` identity, claims the task, opens an `AgentRun`, spawns `claude -p` in a per-task worktree (`~/.ep/worktrees/<task_id>/`) with `--allowed-tools` derived from `required_capabilities`, completes on subprocess exit, DELETEs the `MeshAgent` (FK `ON DELETE SET NULL` on `agentrun.mesh_agent_id` preserves audit).
- **Capability enforcement**: a coarse vocabulary (`shell:read/write`, `fs:read/write`, `vault:read/write`, `edgeplane:read/write`, `web:fetch`, `gh:read/write`) maps to `claude -p --allowed-tools` patterns. Dispatchers declare blast radius via `required_capabilities` (JSON array of capability names) on the `kind='claimable'` task.
