...
use edgeplaned_secrets::{InfisicalConfig, InfisicalProfileMap};
    ...
    match &state.auth_oidc_state {
        None => {
            if let Some(t) = &state.token_masked {
                ...
                lines.push(Line::from(""));
                lines.push(Line::from(Span::styled(
                    "  edgeplane auth logout   to clear the session",
                    theme::dim(),
                )));
...
fn panel_profile(state: &ConfigScreenState) -> Vec<Line<'static>> {
    ...
    if state.content_focused {
        ...
        lines.push(Line::from(Span::styled(
            "  → focus list   edgeplane context add <name> --url <url> to add",
            theme::dim(),
        )));
    }
...
fn panel_version(state: &ConfigScreenState) -> Vec<Line<'static>> {
    ...
    vec![
        Line::from(""),
        Line::from(vec![
            Span::styled("  edgeplane client  ", theme::muted()),
        ...
        Line::from(Span::styled("  edgeplane update", theme::dim())),
    ]
}
