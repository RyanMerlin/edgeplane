/// `mc daemon` — mcd daemon control and work-model commands.
...
pub enum DaemonCommand {
    /// Bring mcd up: install if missing, then start the daemon.
    Up(DaemonUpArgs),
    /// Stop the running mcd daemon (install stays).
    Down,
    /// Remove the mcd binary and systemd unit.
    Uninstall,
    /// Show daemon health: backend reachable, runtimes, watchdog state.
    Status,
    /// Deep health check with individual component results.
    Health,
    /// Upgrade the mcd binary in place.
    Upgrade(DaemonUpgradeArgs),
    /// Print mcd daemon version.
    Version,
...
async fn handle_up(args: DaemonUpArgs, config: &McConfig) -> Result<()> {
    // 1. Check if mcd binary exists.
    let binary = which_mcd();

    if binary.is_none() {
        println!("mcd binary not found in PATH.");
        let install = if args.yes {
            true
        } else {
            prompt_yes_no("Install mcd now? (build from source) [y/N]")
        };
        if install {
    ...
    if is_daemon_running() {
        println!("mcd daemon is already running.");
        return Ok(());
    }
    ...
    println!("Starting mcd daemon…");
    start_daemon_background(&backend_url, &token)?;
    tokio::time::sleep(Duration::from_millis(500)).await;

    if is_daemon_running() {
        println!("mcd daemon started.");
    } else {
        println!(
            "mcd daemon may not have started. Check logs at: journalctl --user -u mcd"
        );
    }
    ...
    if which_binary("systemctl") {
        ...
        if !unit_path.exists() {
            let install = if args.yes {
                ...
                prompt_yes_no("Install systemd user unit so mcd starts on login? [y/N]")
            };
            if install {
    ...
    println!("Run `mc daemon status` to check.");
    Ok(())
}

/// Register this host as a RuntimeNode if no node-state file exists.
/// Best-effort — logs a warning but does not abort `mc daemon up` on failure.
async fn auto_register_node(config: &McConfig) {
    ...
    let client = match crate::client::MissionControlClient::new(config) {
        Ok(c) => c,
        Err(e) => {
            eprintln!("mc daemon up: warning: could not build client for node registration: {e}");
            return;
        }
    };
    ...
    match client.post_json("/runtime/nodes/register", &body).await {
        Ok(resp) => {
            ...
            if !node_id.is_empty() {
                ...
                if let Err(e) = persist_node_state(&state) {
                    eprintln!("mc daemon up: warning: could not save node state: {e}");
                } else {
        ...
        Err(e) => {
            eprintln!("mc daemon up: warning: could not auto-register runtime node: {e}");
        }
    }
}
...
fn install_systemd_unit(unit_path: &std::path::Path) -> Result<()> {
    ...
    let binary = which_mcd().context("mcd binary not found")?;
    let unit = format!(
        "[Unit]\n\
         Description=mcd agent coordination daemon\n\
    ...
    let _ = std::process::Command::new("systemctl")
        ...
        .status();
    let _ = std::process::Command::new("systemctl")
        .args(["--user", "enable", "mcd.service"])
        .status();
    println!(
        "Systemd user unit installed and enabled at {}",
        unit_path.display()
    );
    println!("mcd will start automatically on next login.");
    Ok(())
}
...
fn handle_down() -> Result<()> {
    ...
    if let Ok(pid_str) = std::fs::read_to_string(&pid_path) {
        if let Ok(pid) = pid_str.trim().parse::<u32>() {
            ...
            println!("Sent SIGTERM to mcd daemon (pid {pid}).");
            return Ok(());
        }
    }
    println!("mcd daemon does not appear to be running.");
    Ok(())
}
...
fn handle_uninstall() -> Result<()> {
    ...
    if unit_path.exists() {
        ...
        let _ = std::process::Command::new("systemctl")
            .args(["--user", "disable", "--now", "mcd.service"])
            .status();
    ...
    if removed_binary {
        println!(
            "mcd uninstalled. Config and work dirs are preserved at ~/.mc/mcd*"
        );
    } else {
        println!("mcd binary not found; nothing to remove.");
    }
    Ok(())
}
    ...
    match which_mcd() {
        Some(bin) => {
            ...
            if let Some(o) = out {
                ...
                println!("mcd (version unknown)");
            }
        }
        None => println!("mcd not installed"),
    }
    Ok(())
}
...
async fn handle_agent(cmd: DaemonAgentCommand, client: &MissionControlClient) -> Result<()> {
    match cmd {
        ...
        DaemonAgentCommand::Enroll(a) => {
            if crate::local_db::is_federated() {
                ...
                // what mcd uses to poll `/agents/{public_id}/messages`.
...
fn detect_machine_info() -> Value {
    ...
    let work_dir = crate::config::mc_home_dir().join("mcd").join("work");
...
async fn handle_task(cmd: DaemonTaskCommand, client: &MissionControlClient) -> Result<()> {
    match cmd {
        DaemonTaskCommand::Run(a) => {
            ...
            println!("\nWatch progress: mc daemon task watch {task_id}");
            Ok(())
        }
...
async fn handle_msg(cmd: DaemonMsgCommand, client: &MissionControlClient) -> Result<()> {
    match cmd {
...
#[cfg(unix)]
async fn handle_attach(args: DaemonAttachArgs, client: &MissionControlClient) -> Result<()> {
    ...
    if !socket_path.exists() {
        anyhow::bail!(
            "mcd daemon socket not found at {}.\nIs the daemon running? Try `mc daemon up`.",
            socket_path.display()
        );
    }
...
#[cfg(not(unix))]
async fn handle_attach(_args: DaemonAttachArgs, _client: &MissionControlClient) -> Result<()> {
    anyhow::bail!("`mc daemon attach` is currently only supported on Unix-like hosts");
}
...
async fn handle_upgrade(args: DaemonUpgradeArgs) -> Result<()> {
    ...
    println!("Upgrade complete. Run `mc daemon up` to restart the daemon.");
    Ok(())
}
...
fn pid_file_path() -> std::path::PathBuf {
    std::env::temp_dir().join("mcd.pid")
}


fn is_daemon_running() -> bool {
    // 1. Try the PID file written by `mc daemon start` (foreground / detached
    //    launch from the CLI).
    let pid_path = pid_file_path();
    if let Ok(pid_str) = std::fs::read_to_string(&pid_path) {
        if let Ok(pid) = pid_str.trim().parse::<u32>() {
    ...
    //    (mcd.service) doesn't touch /tmp/mcd.pid, so a responsive
...
fn start_daemon_background(backend_url: &str, token: &str) -> Result<()> {
    let binary = which_mcd().context("mcd binary not found after install attempt")?;
    ...
    let child = std::process::Command::new(&binary)
        ...
        .spawn()
        .context("failed to spawn mcd run")?;
...
fn build_and_install_mcd() -> Result<()> {
    // Find the mcd workspace relative to the mc crate's location.
    // In development, both live in the missioncontrol repo.
    let mcd_dir = locate_mcd_workspace();

    if let Some(dir) = mcd_dir {
        println!("Building mcd from {}…", dir.display());
        let status = std::process::Command::new("cargo")
            .args(["install", "--path", "crates/mcd", "--force"])
            .current_dir(&dir)
            .status()
...
fn locate_mcd_workspace() -> Option<std::path::PathBuf> {
    // Walk up from current exe to find the mcd workspace in development.
...
async fn handle_profile(cmd: DaemonProfileCommand, client: &MissionControlClient) -> Result<()> {
    match cmd {
...
async fn handle_profile_add(a: ProfileAddArgs, _client: &MissionControlClient) -> Result<()> {
    ...
    if profiles.contains_key(&a.name) {
        anyhow::bail!(
            "profile '{}' already exists. Remove it first: mc daemon profile rm {}",
            a.name,
            a.name
        );
    }
    ...
    if state["active_profile"].as_str() == Some(&a.name) {
        ...
        println!("Run `mc daemon use {}` to activate this profile.", a.name);
    }
    Ok(())
}
...
fn handle_profile_list() -> Result<()> {
    ...
    if profiles.is_empty() {
        println!("No profiles saved. Add one: mc daemon profile add <name> --url <url> --token <tok>");
        return Ok(());
    }
...
fn handle_use(a: DaemonUseArgs) -> Result<()> {
    ...
    match &a.name {
        ...
        Some(name) => {
            ...
            if !exists {
                anyhow::bail!(
                    "profile '{}' not found. List profiles with `mc daemon profile list`.",
                    name
                );
            }
