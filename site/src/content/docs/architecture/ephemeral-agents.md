...
description: How mcd spawns, manages, and audits short-lived agent subprocesses for distributed mesh task execution.
---

The **task worker** (`mcd::task_worker`) enables distributed, parallel AI execution within MissionControl's mesh. When a dispatcher submits a `MeshTask`, `mcd` spawns an ephemeral agent subprocess to claim and execute it — without touching any persistent agent session or interrupting operator context.
...
The task worker provides a third path: submit a `MeshTask` to the mesh, let an ephemeral subagent claim it, run it to completion in an isolated context, and report back. The persistent session is never touched. MissionControl retains full visibility and a durable audit trail.
...
`mcd::task_worker` runs two loops per node:
...
4. Allocates a per-task git worktree at `~/.mc/worktrees/<task_id>/` to prevent concurrent git collisions
...
On startup, `mcd` ensures a default `home` domain (name overridable via `MC_HOME_DOMAIN_NAME`) and an `intake` mission exist under it. This is idempotent and soft-fails silently if the control plane is unavailable.
...
| `mc:read` | MissionControl read operations |
| `mc:write` | MissionControl write operations |
...
Despite agent ephemerality, MissionControl retains full visibility:

- **`mc agent list`** — shows parent identity plus active subagent projections
- **`mc task list --status running`** — shows work in progress with `claimed_by_agent_id`
...
| Hazard | Mitigation |
|--------|-----------|
| Concurrent git operations | Per-task worktrees at `~/.mc/worktrees/<task_id>/` — each subagent has its own checkout |
...
submit_mesh_task(
  ...
  required_capabilities = ["shell:read", "mc:read"],
  claim_policy = {"target_profile": "my-profile"}
)

# Via CLI (coming in a future release)
mc daemon task submit --mission-id <id> --prompt "..." --capabilities shell:read,mc:read
```

## See Also

- [Concepts: Entity Reference](/missioncontrol/concepts/entity-reference/) — canonical definitions for `MeshTask`, `MeshAgent`, `AgentRun`
- [Reference: mcd Daemon](/missioncontrol/reference/mcd-daemon/) — daemon configuration and socket interface
- [Architecture: System Overview](/missioncontrol/architecture/overview/) — where `mcd` fits in the overall component map
