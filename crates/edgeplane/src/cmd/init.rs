/// `edgeplane init --repo` — bootstrap this node from a git-backed sync repo.
///
/// Clones the sync repo to ~/.ep/sync/, stores INFISICAL_TOKEN in
/// the OS keyring (if available and set), and writes ~/.ep/config.json
...
/// Write (or merge) `sync_repo` into `~/.ep/config.json`.
///
