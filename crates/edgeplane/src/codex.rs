...
pub async fn run_launch(
    ...
    if !report.ready {
        bail!(
            "{}: not ready; run `edgeplane run codex doctor --fix -p {}`",
            profile,
            profile
        );
    }
...
pub async fn run_exec(
    ...
    if !paths.runtime_home.exists() || !paths.config_path.exists() {
        bail!(
            "{}: runtime is not prepared; run `edgeplane run codex doctor --fix -p {}`",
            profile,
            profile
        );
    }
...
fn inspect_profile(profile: &str, config: &McConfig, repair: bool) -> Result<CodexDoctorReport> {
        ...
        if !raw.contains("[mcp_servers.edgeplane]") {
            ...
            if !raw.contains(&expected) {
                ...
                if repair {
                    write_codex_config(&paths.config_path, config, Some(&raw))?;
                    repaired = true;
                    repaired_actions.push("repaired edgeplane MCP command path".to_string());
                }
            }
        }
    }
    ...
    Ok(CodexDoctorReport {
        ...
        issues,
        repaired_actions,
        suggested_command: format!("edgeplane run codex doctor --fix -p {}", profile),
    })
}
...
fn write_ownership(paths: &CodexPaths) -> Result<()> {
    ...
    let state = OwnershipState {
        ...
        managed_entries: vec!["mcp_servers.edgeplane".to_string()],
...
mod tests {
    ...
    fn strip_block_removes_mc_managed_section_only() {
        let input = r#"a = 1
# edgeplane-codex: edgeplane managed block (begin)
[mcp_servers.edgeplane]
command = \"edgeplane\"
# edgeplane-codex: edgeplane managed block (end)
b = 2
"#;
        let out = strip_mc_managed_block(input);
        assert!(out.contains("a = 1"));
        assert!(out.contains("b = 2"));
        assert!(!out.contains("mcp_servers.edgeplane"));
    }
    ...
    fn doctor_json_contract_contains_expected_top_level_fields() {
        let report = CodexDoctorReport {
            ...
            suggested_command: "edgeplane run codex doctor --fix".to_string(),
        };
    ...
    fn write_codex_config_repairs_missing_managed_block_and_preserves_user_content() {
        ...
        assert!(repaired.contains("[mcp_servers.edgeplane]"));
        assert!(repaired.contains("EP_BASE_URL"));
    }
