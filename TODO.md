...
- [x] `integrations/mc-mesh/scripts/demo_three_agents.sh`: end-to-end dependency chain
...
- [x] mc-mesh work loop: adaptive backoff (5s→30s), `depends_on`/`produces`/`consumes`
...
- [x] `mc-mesh` node heartbeat: `node_id` in `DaemonConfig`; background task sends
...
- [x] `mc secrets infisical` Universal Auth: `client_id + client_secret → token exchange`
      fully implemented in `mc-mesh-secrets/src/client.rs` with in-process token cache (2026-04-28)
