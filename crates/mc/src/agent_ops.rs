...
//! ## Transport
//!
//! Local: Unix socket `~/.missioncontrol/mgmt.sock`, newline-delimited
//! JSON-RPC 2.0, no auth (filesystem perms gate access).
//! Remote: existing controlplane HTTP via [`MissionControlClient`].
...
pub async fn run_signal(args: SignalArgs, client: &MissionControlClient) -> Result<()> {
    if args.remote {
    ...
    // Auto-resolve: ask mcd if it knows the agent locally.
...
pub async fn run_cancel(args: CancelArgs, client: &MissionControlClient) -> Result<()> {
    if args.remote {
...
pub async fn run_describe(args: DescribeArgs, client: &MissionControlClient) -> Result<()> {
...
pub async fn run_list(args: ListArgs, client: &MissionControlClient) -> Result<()> {
    ...
    if matches!(args.source, ListSource::Local | ListSource::All) {
        local_agents = match list_local().await {
            Ok(v) => v,
            Err(e) => {
                // Local mcd unavailable shouldn't kill `--source all`.
...
async fn signal_remote(agent_id: &str, content: &str, client: &MissionControlClient) -> Result<()> {
    // Forward to the existing mc signal logic (which knows how to look up
...
async fn cancel_remote(_agent_id: &str, _client: &MissionControlClient) -> Result<()> {
    bail!(
        "remote cancel not yet implemented for controlplane agents. \
         Use `mc agent signal <id> --content 'cancel'` to send a cancel prompt, \
         or implement Cancel routing through the controlplane in a follow-up PR."
    )
}

async fn describe_remote(agent_id: &str, client: &MissionControlClient) -> Result<Value> {
...
async fn list_remote(client: &MissionControlClient) -> Result<Vec<Value>> {
    let v: Value = client
        .get_json("/agents")
        .await
        .context("GET /agents")?;
...
fn mgmt_socket_path() -> std::path::PathBuf {
    // Mirrors mcd_core::paths::mgmt_socket_path() — `~/.missioncontrol/mgmt.sock`.
...
fn print_describe_human(envelope: &Value) {
    ...
    if let Some(m) = agent.get("mission_id").and_then(|v| v.as_str()) {
        if !m.is_empty() {
            println!("mission_id:    {m}");
        }
    }
