# mc-mesh: Controlplane-Driven Enrollment & State/Config Separation

**Date:** 2026-05-10
**Status:** Design — pending sign-off
**Author:** mc-engineer (Aria) + Merlin
**Supersedes (in part):** Sections of `docs/plans/mc-mesh-persistent-session-architecture.md` describing yaml-based agent enrollment
...
The same rule applies to mc-mesh.yaml: **it must not be the authority for mission/agent assignment**. Today it is. Below is what's broken, what the target looks like, and a phased path to get there without throwing away Layers 1–3.

---

## What's broken today

Concrete evidence in the current `DaemonConfig` (`integrations/mc-mesh/crates/mc-mesh/src/config.rs`):
...
| "What is mc-mesh doing?" | Acting as a partial agent registry | Per docs: it's a **per-node arm**, like kubelet — claims work, doesn't define work |
| "Reassign aria-work to a new mission?" | Edit yaml on every node, restart daemon | Should be `mc mesh agent reassign` → controlplane UPDATE → daemon rebalances live |
...
Layers 1–3 are not undermined by this. The `mc-mesh-acp` client, the `claude_agent_acp` runtime, and `acp_session_supervisor` are correct building blocks. What's wrong is **what feeds them**: yaml-defined enrollment.
...
### New `mc-mesh.yaml` shape

Five lines, max:

```yaml
# ~/.mc/mc-mesh.yaml  (or /etc/mc-mesh/agent.yaml)
...
### State file: `~/.mc/mc-mesh.state.json` (0600, daemon-managed)
...
**Written by:**
- `mc mesh node register --bootstrap-token <jt_…>` (explicit user action), OR
- the daemon's first-run path if it sees a `MC_MESH_BOOTSTRAP_TOKEN` env var

**Read by:** the daemon at every startup. If missing → daemon refuses to start with a clear "run `mc mesh node register` first" message.
...
3. Read state file → node_id, attach_secret
   └─ if missing/incomplete: error out unless MC_MESH_BOOTSTRAP_TOKEN is set,
...
2. **MeshAgent record** — `meshagent.capabilities` column (already exists, settable via API). User adjusts via `mc mesh agent capabilities <id> --add foo --remove bar`.
...
The capability dispatcher (`mc-mesh-core::capability_dispatcher`) unions all three when matching against `task.required_capabilities`.

### Mission reassignment flow

```
admin: mc mesh agent reassign aria-work --mission new-research
  ...
  →  mc-mesh on the host running aria-work:
        receive WS event
...
### Phase 4b — mc-mesh: state file + drop node_id/attach_secret from yaml

- [ ] New `state.rs` module: read / write `~/.mc/mc-mesh.state.json` with 0600 perms, schema versioning, atomic rename.
- [ ] `mc mesh node register --bootstrap-token <jt_…>` → POSTs to `/runtime/nodes/register`, captures the response, writes state file.
...
### Phase 4c — mc-mesh: pull agent assignments from controlplane
...
- [ ] `AgentEntry` and `MissionEntry` types removed from `config.rs`. Their replacement is the `MeshAgentRecord` returned by the controlplane (already exists in `mc-mesh-core::types`).
...
- [ ] `mc mesh agent reassign <id> --mission <new>` CLI surface (controlplane endpoint already supports the underlying mutation).
...
- [ ] `mc-mesh-acp::Agent` exposes `InitializeResponse.agentCapabilities` (already captured internally, just needs an accessor).
...
1. **Bootstrap token UX.** `MC_MESH_BOOTSTRAP_TOKEN` env var on first daemon start, OR explicit `mc mesh node register --bootstrap-token <jt_…>` only? I lean explicit — env-var auto-register is too magic. Confirm?
...
5. **What about `attach_bind_addr`?** This stays in yaml today (genuine local infra). Confirm — or should it also live in state, configured via `mc mesh node register --attach-bind 0.0.0.0:8009`?
...
- A user can run `mc mesh node register --bootstrap-token <jt_…>` once on a node.
- They never see `node_id` or `attach_secret` in yaml.
- They run `mc mesh agent enroll --node <node-id> --mission <mission-id> --runtime claude_agent_acp --supervision persistent --profile-path /…` to schedule an agent.
- They run `mc mesh agent reassign <agent-id> --mission <new>` and the daemon rebalances live, no restart.
- `mc mesh.yaml` on a fresh node is 5 lines.
- The Aria fleet (5 profiles, 1 host today) runs as a single `mc-mesh.service` per host with all assignment owned by the controlplane.
