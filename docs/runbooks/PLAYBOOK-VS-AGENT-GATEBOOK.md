...
4. Capture the deterministic failure mode (HTTP 429, handshake 503, missing docs) by bundling the run-level `summary.json`, `pressure-results.jsonl`, and relevant `edgeplane daemon`/`mcpd` logs, then rerun once the blocker resolves.
