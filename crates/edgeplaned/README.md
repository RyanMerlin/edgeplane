...
Work-first agent coordination daemon for Edgeplane. Dispatches tasks to agent runtimes (claude-code, codex, gemini), streams structured progress events, and coordinates agents around a shared mission/kluster.
...
crates/edgeplaned/
├── crates/
│   ├── edgeplaned/              # daemon binary
│   ├── edgeplaned-core/         # shared types, client, progress events, AgentRuntime trait
│   ├── edgeplaned-work/         # task dispatch, DAG, claim, messaging, watchdog
│   └── edgeplaned-runtimes/     # claude-code, codex, gemini AgentRuntime impls
├── systemd/              # edgeplaned.service unit template
└── scripts/
    ├── install.sh        # build and install daemon + edgeplane
...
`edgeplaned` is a headless daemon. All user interaction is through the `edgeplane daemon` subcommand group.

```sh
edgeplane daemon up                          # install and start daemon
edgeplane daemon status                      # daemon health + active agents
edgeplane daemon runtime install claude-code # ensure runtime CLI is available
edgeplane daemon runtime install codex
edgeplane daemon runtime install gemini

edgeplane daemon agent enroll --mission <id> --runtime claude-code
edgeplane daemon agent ls --mission <id>

edgeplane daemon task run <kluster> --title "build it" --runtime claude-code
edgeplane daemon task watch <task-id>        # live progress stream
edgeplane daemon task show <task-id>         # full detail + replay
edgeplane daemon attach <agent-or-task-id>   # PTY into a running agent

edgeplane daemon watch --kluster <id>        # unified live feed
edgeplane daemon watch --mission <id>
...
The work model is mounted at `/work/` in the Edgeplane backend (`backend/app/routers/work.py`). Key endpoints:
...
```sh
crates/edgeplaned/scripts/e2e-test.sh
```

## Installation

```sh
crates/edgeplaned/scripts/install.sh       # builds from source, installs to ~/.cargo/bin/
edgeplane daemon up                              # starts daemon, optionally installs systemd unit
```

Or via the `edgeplane` bootstrap in `crates/edgeplane/`:
```sh
