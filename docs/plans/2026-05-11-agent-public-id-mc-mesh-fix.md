# Agent public_id + mc-mesh Delivery Fix

**Date:** 2026-05-11  
**Author:** aria-merlinlabs  
**Priority:** High — blocks all mc-mesh message delivery to Claude Code agents

---

## Problem

Two bugs prevent mc-mesh from delivering messages to running Claude Code agents:

1. **`/work/` prefix hardcoded in mc-mesh** — all API calls hit `/work/agents/...`, `/work/klusters/...` etc., but mc-controlplane serves these routes without any prefix → connection closed
2. **Agent ID type mismatch** — mc-mesh uses internal UUIDs for agent identity; mc-controlplane uses `i32` integer IDs → `"Cannot parse UUID to i32"`
...
Include `public_id` in all agent responses so mc-mesh can record it after enrollment.

---

### Part 2 — mc-mesh: Strip `/work/` + use public_id

**2.1 Strip the `/work/` prefix — single-place fix**

`integrations/mc-mesh/crates/mc-mesh-core/src/client.rs`
...
- `mc-mesh/src/task_loop.rs:405` — `/work/agents/{id}/messages`
- `mc-mesh-work/src/task.rs:43,66` — `/work/klusters/...`, `/work/tasks/...`
- `mc-mesh-core/src/client.rs:101` — `/work/missions/{id}/roster`

**2.2 Use public_id from enrollment**

`mc-mesh/src/daemon.rs` — `agent_spec_from_json()` (lines 827-881):
...
`mc-mesh/src/local_registry.rs` (line 59): update upsert to store `public_id` from controlplane response, not the local UUID.

---

### Part 3 — mc CLI: Display + routing

**3.1** — `mc mesh agent ls`: show `public_id` as primary identifier, drop numeric `id` from default output  
...
Step 3: mc-mesh — client prefix fix (unblocks ALL /work/ routes)
Step 4: mc-mesh — enrollment reads public_id, stores it, uses for polls
...
**Suggested branch:** `feat/agent-public-id` covering Steps 1-4 together — they need to ship atomically since mc-mesh enrollment must read a public_id that the controlplane generates.
...
mc agent remote message --agent-id aria-merlinlabs-x1y2z3a4 --to-agent-id aria-work-qwn5eb33 --content "wake up"
  ...
  → mc-mesh polls /agents/aria-work-qwn5eb33/messages (no /work/ prefix, correct ID)
  → mc-mesh injects into Claude Code session via ACP
...
| `integrations/mc-mesh/crates/mc-mesh-core/src/client.rs` | Strip api_prefix |
| `integrations/mc-mesh/crates/mc-mesh/src/daemon.rs` | Read public_id from enrollment |
| `integrations/mc-mesh/crates/mc-mesh/src/local_registry.rs` | Store public_id |
