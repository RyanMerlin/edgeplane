//! `edgeplane agent supervise watch` — live fleet dashboard.
//!
...
//!     `events.subscribe`)
//!
//! Both panes read from the local edgeplaned over the Unix socket (no controlplane
        ...
        loop {
            ...
            match stream_events(Arc::clone(&state_stream), tail_size).await {
                Ok(()) => {
                    ...
                    // edgeplaned hung up — back off briefly then retry.
                }
                Err(e) => {
                    let mut s = state_stream.lock().unwrap();
...
async fn stream_events(state: Arc<Mutex<State>>, tail_size: usize) -> Result<()> {
    ...
    let stream = tokio::net::UnixStream::connect(&path)
        .await
        .with_context(|| format!("connect to {} (is edgeplaned running?)", path.display()))?;
...
fn apply_event(s: &mut State, frame: &Value, tail_size: usize) {
    ...
    let line_text = match kind {
        "unit_restarted" => {
            ...
            let result = frame.get("result").and_then(|v| v.as_str()).unwrap_or("?");
            format!(
                "{}  RESTART    {agent} reason={reason} result={result}",
                short_time(at)
            )
        }
...
fn render(f: &mut ratatui::Frame, s: &RenderSnapshot) {
    ...
    let status_line = match &s.stream_status {
        ...
        StreamStatus::Closed => Span::styled(
            "● closed (reconnecting)",
    ...
    // Events tail.
    let tail_lines: Vec<Line> = s
        .tail
        .iter()
        .rev()
        .take(chunks[2].height.saturating_sub(2) as usize)
        .rev()
        .map(|l| Line::from(Span::styled(l.text.clone(), Style::default().fg(l.color))))
        .collect();
    let tail_widget = Paragraph::new(tail_lines).block(
        Block::default()
            .borders(Borders::ALL)
            .title(" Events (live) "),
    );
...
mod tests {
    ...
    fn apply_event_updates_agent_row_and_tail() {
        ...
        assert_eq!(
            s.agents[0].last_event_label.as_deref(),
            Some("RESTART/started")
        );
