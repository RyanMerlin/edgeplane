...
/// Insert a `kind='claimable'` row of the unified `task` table (`meshtask`
/// was renamed to `task` by migration 0014) with an explicit status (any
/// value, including terminal states not covered by the shared
/// `common::seed_*` helpers).
async fn seed_task_with_status(
    ...
    sqlx::query(
        "INSERT INTO task \
         (id, public_id, mission_id, domain_id, kind, title, description, input_json, claim_policy, \
...
async fn load_mission_workspace_excludes_terminal_meshtasks() {
    ...
    let _finished_id =
        seed_task_with_status(&pool, &ctx.mission_id, &ctx.domain_id, "finished").await;
    let _cancelled_id =
        seed_task_with_status(&pool, &ctx.mission_id, &ctx.domain_id, "cancelled").await;
