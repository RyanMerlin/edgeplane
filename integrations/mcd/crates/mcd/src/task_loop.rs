...
async fn stream_and_heartbeat(
    mut stream: futures::stream::BoxStream<'static, mcd_core::progress::ProgressEvent>,
    ...
    while let Some(event) = stream.next().await {
        ...
        if event.event_type == mcd_core::progress::ProgressEventType::Error {
            success = false;
        }
