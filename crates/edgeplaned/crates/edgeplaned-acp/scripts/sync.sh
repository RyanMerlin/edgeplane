...
# - If different: updates schema.json + VERSION, then runs `cargo test -p edgeplaned-acp`.
...
echo "==> rebuilding + testing edgeplaned-acp (build.rs will regenerate types)"
(cd "$WORKSPACE_DIR" && cargo nextest run -p edgeplaned-acp)

echo
echo "Sync complete. Review the changes in:"
echo "  $SCHEMA_FILE"
echo "  $VERSION_FILE"
echo "  (and the regenerated types via \`cargo expand -p edgeplaned-acp schema\`)"
