...
///      c. Sets up the mount namespace: tmpfs root, RO bind of pinned binary + ldd libs, proc, tmpfs /tmp, tmpfs /home/mcd.
    ...
    if val.trim() == "1" {
        return Err(SandboxError::Isolation(
            "AppArmor is blocking unprivileged user namespaces \
             ...
             \x20  sudo cp assets/apparmor/mcd-worker /etc/apparmor.d/mcd-worker\n\
             \x20  sudo apparmor_parser -r /etc/apparmor.d/mcd-worker\n\
...
pub fn enter_jail(config: &JailConfig) -> Result<()> {
    ...
    if child_pid == 0 {
        ...
        if let Err(e) = unshare_namespaces() {
            eprintln!("[mcd-jail] unshare failed: {e}");
        ...
        if let Err(e) = setup_mount_namespace(&root_path, config) {
            eprintln!("[mcd-jail] mount namespace failed: {e}");
            unsafe { libc::_exit(127) };
        }

        // Step 6: pivot_root
        if let Err(e) = pivot_into(&root_path) {
            eprintln!("[mcd-jail] pivot_root failed: {e}");
            unsafe { libc::_exit(127) };
        }

        // Step 7: NO_NEW_PRIVS
        if let Err(e) = set_no_new_privs() {
            eprintln!("[mcd-jail] no_new_privs failed: {e}");
            unsafe { libc::_exit(127) };
        }
...
fn setup_mount_namespace(root: &Path, config: &JailConfig) -> Result<()> {
    ...
    for dir in &["proc", "tmp", "home", "home/mcd", "usr", "lib", "lib64", "bin", "dev"] {
...
fn drop_capabilities() -> Result<()> {
    ...
    if ret != 0 {
        ...
        eprintln!("[mcd-worker] capset warning: {}", std::io::Error::last_os_error());
    }
    Ok(())
}
...
mod tests {
    ...
    fn test_resolve_nonexistent() {
        assert!(resolve_and_hash_binary("this_binary_does_not_exist_mcd").is_err());
    }
