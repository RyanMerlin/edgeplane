//! `edgeplaned doctor` — read-only health check.
//!
...
//! by a live edgeplaned" (the expected steady state). Non-zero if anything looks
//! actionably wrong.
...
pub async fn run() -> Result<()> {
    ...
    println!("edgeplaned doctor — {}", chrono::Local::now().to_rfc3339());
    println!();
...
fn check_lock() -> Finding {
    let path = edgeplaned_core::paths::lock_file_path();
        ...
        return Finding {
            ...
            detail: format!(
                "lock file not yet created at {path_display}\n\
                 (expected if edgeplaned has never run; will appear on first start)"
            ),
        };
    }
    ...
    match file.try_lock_exclusive() {
        Ok(()) => {
            ...
            Finding {
                ...
                title: "singleton lock".into(),
                detail: format!(
                    "lock at {path_display} is NOT held — no edgeplaned is currently running.\n\
                     Start with `systemctl --user start edgeplaned.service` or `edgeplaned run`."
                ),
            }
        }
...
/// - AddrInUse → listener present. Check if it's the running edgeplaned via the
        ...
        Err(e) if e.kind() == std::io::ErrorKind::AddrInUse => {
            if mcd_running {
                Finding {
                    severity: Severity::Ok,
                    title: format!("port {name} ({addr})"),
                    detail: "bound (expected — owned by the running edgeplaned)".into(),
                }
            } else {
                Finding {
                    severity: Severity::Error,
                    title: format!("port {name} ({addr})"),
                    detail: format!(
                        "port is in use but no edgeplaned holds the singleton lock — something \
                         else owns this port.\n\
...
fn check_registry() -> Finding {
    let path = edgeplaned_core::paths::registry_db_path();
