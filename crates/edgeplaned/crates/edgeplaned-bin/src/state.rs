//! Daemon-managed state file: `~/.ep/state.json` (mode 0600).
//!
//! ## Schema history
//!
//! ### v1 (Phase 4b)
...
pub struct ProfileAuth {
    /// Auth kind: `"oidc"` (session token from `edgeplane auth login`) or `"service"`
    /// (long-lived machine credential issued by edgeplane-tower at registration).
    pub kind: String,
...
impl DaemonState {
    /// Default state-file path: `~/.ep/edgeplaned/state.json`.
    pub fn default_path() -> Result<PathBuf> {
        Ok(edgeplaned_core::paths::state_file_path())
    }
        ...
        if version > STATE_SCHEMA_VERSION {
            tracing::warn!(
                "state file at {} is schema_version={version} but daemon supports up to {}; \
                 loading as-is — upgrade edgeplaned if this causes issues.",
                path.display(),
