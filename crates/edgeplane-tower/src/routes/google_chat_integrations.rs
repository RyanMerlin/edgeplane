...
pub fn router() -> Router<Arc<AppState>> {
    Router::new().route("/integrations/google-chat/events", post(google_chat_events))
}
...
fn verify_google_chat(headers: &axum::http::HeaderMap, body: &[u8]) -> VerifyResult {
    let signing_secret = std::env::var("GOOGLE_CHAT_SIGNING_SECRET").unwrap_or_default();
    let verification_token = std::env::var("GOOGLE_CHAT_VERIFICATION_TOKEN").unwrap_or_default();

    if !signing_secret.is_empty() {
        let timestamp = headers
            .get("x-edgeplane-timestamp")
            .and_then(|v| v.to_str().ok())
            .unwrap_or("");
        let signature = headers
            .get("x-edgeplane-signature")
            .and_then(|v| v.to_str().ok())
            .unwrap_or("");
        ...
        if !constant_time_eq(&expected, signature) {
            return VerifyResult::Err(
                ...
                serde_json::json!({"code": "google_chat_signature_invalid", "reason": "hmac_invalid"}),
            );
        }
    ...
    if !verification_token.is_empty() {
            ...
            .and_then(|v| v.to_str().ok())
        {
            ...
            return VerifyResult::Err(
                StatusCode::UNAUTHORIZED,
                serde_json::json!({"code": "google_chat_signature_invalid", "reason": "token_missing_or_invalid"}),
            );
        }
        // Try payload token
    ...
    VerifyResult::Err(
        StatusCode::UNAUTHORIZED,
        serde_json::json!({"code": "google_chat_signature_invalid", "reason": "google_chat_verification_token_missing"}),
    )
}

// --- handler ---

async fn google_chat_events(
    State(state): State<Arc<AppState>>,
    headers: axum::http::HeaderMap,
    body: axum::body::Bytes,
) -> impl IntoResponse {
    match verify_google_chat(&headers, &body) {
    ...
    if event_type == "ADDED_TO_SPACE" {
        return Json(serde_json::json!({
            "text": "Edgeplane connected. Bind this space to a domain to enable actions."
        }))
        .into_response();
    }

    let space = payload.get("space").and_then(|v| v.as_object());
    ...
    let result = sqlx::query(
        "INSERT INTO chatinboundreceipt (provider, event_key, event_type, created_at) \
         VALUES ('google_chat', $1, 'event', now()) \
    ...
    match result {
        ...
        Err(e) => {
            tracing::error!("google_chat_events insert receipt: {e}");
