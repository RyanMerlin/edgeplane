...
use anyhow::Result;
use edgeplaned_core::capability_dispatcher::{CapabilityDispatcher, DispatchRequest};
use edgeplaned_core::paths;
use edgeplaned_core::types::AgentSignal;
use edgeplaned_packs::PackRegistry;
...
pub struct AgentOpsHandle {
    ...
    pub supervisor_events:
        Option<tokio::sync::broadcast::Sender<edgeplaned_core::types::SupervisorEvent>>,
}
...
async fn stream_supervisor_events<W>(
    sender: Option<&tokio::sync::broadcast::Sender<edgeplaned_core::types::SupervisorEvent>>,
    id: Value,
    writer: &mut W,
) -> Result<()>
where
    ...
    let lookup = ops
        ...
        .with_agent(&agent_id, |supervised| {
            (
                ...
                edgeplaned_core::types::AgentHandle {
...
/// exists in any local registry source. The `edgeplane` CLI calls this first when
/// auto-resolving `edgeplane agent signal/attach/describe <id>` — found → local
    ...
    match outcome {
        Ok(Ok((agent_id, source, systemd_service, exit))) => {
            ...
            if let Some(tx) = events_tx {
                let _ = tx.send(edgeplaned_core::types::SupervisorEvent::UnitRestarted {
        ...
        Ok(Ok(affected)) => {
            ...
            if let Some(tx) = events_tx {
                for (source, agent_id) in &affected {
                    let ev = if paused {
                        edgeplaned_core::types::SupervisorEvent::SupervisePaused {
                            agent_id: agent_id.clone(),
                            source: source.clone(),
                            at: chrono::Utc::now().to_rfc3339(),
                        }
                    } else {
                        edgeplaned_core::types::SupervisorEvent::SuperviseResumed {
                            agent_id: agent_id.clone(),
                            source: source.clone(),
                            at: chrono::Utc::now().to_rfc3339(),
                        }
