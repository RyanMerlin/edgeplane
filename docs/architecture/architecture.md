...
- **Triage loop**: polls the intake mission for unscoped tasks. Calls `aria goose` (local Qwen3-30B-A3B) for categorization; on confidence ≥ threshold, creates a child `MeshTask` with `parent_task_id` chain and `claim_policy.target_profile` set (which the claim loop then picks up). Below threshold → marks the intake task `blocked` (discoverable via `edgeplane task ls --status blocked`); if `task_worker_surface_command` is configured, also invokes that command with `<task_id> <title> <reason>` so deployments can chain external alerts (vault note, Slack, etc.) without EdgePlane encoding any particular interface.
