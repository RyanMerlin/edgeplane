//! `mc agent cron` — inspection + reload for mcd's cron scheduler.
//!
//! All verbs route through the mgmt_gateway over the Unix socket
//! (`~/.missioncontrol/mcd/mgmt.sock`). Phase 4 of the daemon-absorption
//! plan. Job definitions live in `~/.mc/mcd/cron.toml` — operators edit
