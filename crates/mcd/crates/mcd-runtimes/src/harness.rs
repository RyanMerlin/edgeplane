//! Shared harness helper — writes the mcd capabilities block to an agent's
//! config file. Idempotent: a marker pair ensures the block appears exactly once.

use anyhow::{Context, Result};
use std::path::Path;

const MARKER_START: &str = "<!-- mcd capabilities -->";
const MARKER_END: &str = "<!-- /mcd capabilities -->";
...
pub fn write_capabilities_block(path: &Path) -> Result<()> {
    ...
    if path.exists() {
        ...
        let new_content = match (start_pos, end_pos) {
            ...
            _ => anyhow::bail!(
                "{}: file contains only one mcd marker or markers are inverted; \
                 refusing to modify",
                path.display()
            ),
        };
