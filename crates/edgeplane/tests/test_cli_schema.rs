...
fn discover_stdout_is_valid_json() {
    ...
    assert!(
        ...
        "edgeplane discover --deep should exit 0"
    );
    let stdout = String::from_utf8(output.stdout).expect("non-utf8 output");
    let parsed: serde_json::Value =
        serde_json::from_str(&stdout).expect("discover --deep output should be valid JSON");
...
fn discover_includes_agent_subcommand() {
    ...
    assert!(
        agent.is_some(),
        "should have 'agent' in top-level subcommands"
    );
...
fn discover_path_drills_into_subtree() {
    ...
    assert!(
        output.status.success(),
        "edgeplane discover agent should exit 0"
    );
    let stdout = String::from_utf8(output.stdout).unwrap();
    let parsed: serde_json::Value = serde_json::from_str(&stdout).unwrap();
