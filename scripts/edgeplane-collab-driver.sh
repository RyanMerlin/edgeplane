...
DURATION_SEC="${EP_COLLAB_DURATION_SEC:-600}"
POLL_SEC="${EP_COLLAB_POLL_SEC:-5}"
STACK_PROFILE="${EP_STACK_PROFILE:-full}"
DOMAIN_ID="${EP_COLLAB_DOMAIN_ID:-}"
MISSION_ID="${EP_COLLAB_MISSION_ID:-}"
...
if [[ ! -f "$SCENARIO_FILE" ]]; then
  echo "EP_COLLAB_SCENARIO_FILE not found: $SCENARIO_FILE" >&2
  exit 2
fi
...
create_seed_if_needed() {
  ...
  domain_name="edgeplane-improves-edgeplane-${scenario_name}-${RUN_ID}"
...
EP_STACK_PROFILE=${STACK_PROFILE}
