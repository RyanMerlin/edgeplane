# edgeplaned: Standalone-First with Controlplane Profiles
...
`2026-05-10-edgeplaned-controlplane-driven-enrollment.md`. A real local
...
edgeplaned is an **agent runtime**, full stop. It always works locally. It
...
This is consistent with `EDGEPLANE_PHILOSOPHY.md` — "coordination
truth stays in Edgeplane" — because in standalone mode, **edgeplaned
itself plays the Edgeplane role for its local scope.** Truth lives
...
| State | Description |
|---|---|
| **standalone** | No active profile. SQLite is source of truth. `edgeplane daemon agent enroll` writes locally. |
...
### `~/.ep/edgeplaned.db` (SQLite, mode 0600)
...
### `~/.ep/edgeplaned.state.json` (extends Phase 4b)
...
```json
{
  ...
  "profiles": {
    "work": {
      "url": "https://edgeplane.work.example.com",
...
**Hard rule:** `edgeplaned` (the daemon binary) is kubelet — it has no
user-facing subcommands. All operator surfaces go through `edgeplane daemon …`
(the kubectl-equivalent). The `edgeplaned` binary is left with:

- `edgeplaned run` — daemon entry, called by systemd
- `edgeplaned get-secret` — internal, called by spawned agent subprocesses
  via the secrets socket
- `edgeplaned version`

The user-facing surfaces below all live in `crates/edgeplane/src/mesh.rs`
(where the existing `MeshCommand` enum already gathers `edgeplane daemon up`,
`edgeplane daemon agent`, `edgeplane daemon task`, etc.). Phase 5c extends that enum.

```
edgeplane daemon profile add <name> --url <…> --bootstrap-token <jt_…>
    Calls /runtime/nodes/register against the URL, stores the result
    plus URL+auth as a saved profile. Does NOT activate it.
    Replaces the temporary `edgeplaned node-register` subcommand from
    Phase 4b — that one moves into `edgeplane daemon profile add`'s
    implementation and the edgeplaned binary loses the subcommand.

edgeplane daemon profile list
    Lists saved profiles + which is active. Reads state.json.

edgeplane daemon profile remove <name>
    Drops the profile entry. If active, prints "restart daemon to
    take effect" and clears active_profile.

edgeplane daemon profile rename <old> <new>
    Cosmetic rename. Useful for the v1→v2 migration default ("default"
    → something meaningful).

edgeplane daemon use <name>           # activate saved profile
edgeplane daemon use --standalone     # deactivate (no controlplane)
...
edgeplane daemon agent reassign <id> --mission <new>     # Phase 4d-b — same logic
edgeplane daemon agent unenroll <id>                     # Phase 4d-b — same logic
edgeplane daemon agent ls                                # query SQLite (both modes)
```

These already exist in skeleton form (`MeshAgentCommand::{Ls, Enroll}`)
in `crates/edgeplane/src/mesh.rs:88`. Phase 5c fills in `Reassign`,
`Unenroll`, and the `Profile` subcommand tree, and rewires `Enroll` to
branch on standalone vs. federated.

### How `edgeplane` reaches the local SQLite + daemon

Two interfaces. `edgeplane` chooses based on what it's doing:

- **Direct file I/O** (when daemon may not be running): reads/writes
  `~/.ep/edgeplaned.state.json`, `~/.ep/edgeplaned.db`. Used by:
...
- **edgeplaned mgmt socket** (when daemon is running, for hints):
  `edgeplane daemon agent enroll` etc. send a "reconcile" message to
  `~/.ep/edgeplaned-mgmt.sock` so the daemon re-runs its reconciler
  immediately. Best-effort; the 60s poll catches anything missed.

The federated-mode mutations (`agent enroll`, `agent reassign`, etc.)
hit the controlplane HTTP API directly from `edgeplane` — no daemon round-trip
...
```
1. Load edgeplaned.yaml (small)
2. Resolve auth (edgeplane auth → backend_url + token IF the active profile
   doesn't override)
3. Load state.json
...
```
$ edgeplaned use personal
Active profile: personal (was: work)
Restart the daemon to apply: systemctl --user restart edgeplaned
```

On daemon restart, it sees the new active_profile, registers under that
profile's identity (already done at `edgeplaned profile add` time), pulls
...
The edgeplane TUI gains a Profiles tab:

```
┌─ Profiles ──────────────────────────────────────────────────┐
│                                                             │
│  ★ work       https://edgeplane.work.example.com   12 agents       │
│    personal   https://edgeplane.merlin.dev          3 agents       │
│    homelab    http://edgeplane:8008     5 agents       │
...
the TUI = same as `edgeplaned use <name>`.
...
- [ ] `edgeplaned-local-registry` module: open SQLite, run migrations,
      CRUD on `agent` table
- [ ] Daemon reads from SQLite when no active profile
- [ ] `edgeplaned agent enroll/reassign/unenroll/list` write to SQLite in
      standalone mode
...
### 5c — Profile management CLI (in `edgeplane`, not `edgeplaned`)
- [ ] Extend `crates/edgeplane/src/mesh.rs::MeshCommand` with:
      `Profile(MeshProfileCommand)` and `Use(MeshUseArgs)`.
- [ ] `MeshProfileCommand::{Add, List, Remove, Rename}` implementations
      that read/write `~/.ep/edgeplaned.state.json` directly.
- [ ] `MeshUseArgs` writes `active_profile`. Prints daemon-restart hint.
- [ ] Wire `edgeplane daemon agent {enroll,reassign,unenroll,ls}` to branch on
      "active profile present?" — standalone path writes SQLite,
      federated path POSTs to controlplane.
- [ ] **Remove** the `node-register` subcommand from the `edgeplaned`
      binary added in Phase 4b. Its body moves into `edgeplane daemon profile add`'s
      handler in `crates/edgeplane/src/`. Update `Phase 4b`'s state-file
      module so the edgeplane binary can call it (move it from
      `crates/edgeplaned/src/state.rs` to a shared crate, e.g.
      `edgeplaned-core::state` or a new `edgeplaned-state` crate, so both `edgeplane`
      and `edgeplaned` link against it).
...
| **4b** state file for node identity | becomes per-profile (schema v2). Migration from v1 is mechanical. The `edgeplaned node-register` subcommand introduced there is **removed** in Phase 5c — it was the wrong split and never should have lived on the daemon binary. Its logic moves into `edgeplane daemon profile add`. |
...
1. **Migration default for old `state.json` v1 → v2.** The v1 file holds a
   ...
   rename via `edgeplaned profile rename default <better>`.
...
4. **Scope of secrets across profiles.** The edgeplaned-secrets broker
...
- Web UI for profile management (edgeplane TUI is enough for V1)
...
- **`edgeplane daemon health`** (exists, shallow) — extend to cover local registry
  integrity: can open SQLite, schema version matches, all enrolled agents
  still have valid runtime binaries on PATH.
- **`edgeplane doctor`** — new top-level command (or `edgeplane daemon doctor`). Checks:
  ...
  4. edgeplaned daemon running + responding to mgmt socket
  5. All enrolled agents have their runtime binary available
  6. No orphaned PIDs / stale sockets in `~/.ep/`
...
- Tie to `edgeplane daemon status` output so health problems surface without extra commands.
