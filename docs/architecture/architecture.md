...
- **Bootstrap** (mcd startup): ensures a fleet-operations mission (`aria-fleet-ops` by default) and an `intake` kluster under it. Idempotent, soft-fail.
