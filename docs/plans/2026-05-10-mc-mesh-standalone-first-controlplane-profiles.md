# mc-mesh: Standalone-First with Controlplane Profiles
...
`2026-05-10-mc-mesh-controlplane-driven-enrollment.md`. A real local
...
mc-mesh is an **agent runtime**, full stop. It always works locally. It
...
truth stays in MissionControl" — because in standalone mode, **mc-mesh
...
| State | Description |
|---|---|
| **standalone** | No active profile. SQLite is source of truth. `mc mesh agent enroll` writes locally. |
...
### `~/.mc/mc-mesh.db` (SQLite, mode 0600)
...
### `~/.mc/mc-mesh.state.json` (extends Phase 4b)
...
**Hard rule:** `mc-mesh` (the daemon binary) is kubelet — it has no
user-facing subcommands. All operator surfaces go through `mc mesh …`
(the kubectl-equivalent). The `mc-mesh` binary is left with:

- `mc-mesh run` — daemon entry, called by systemd
- `mc-mesh get-secret` — internal, called by spawned agent subprocesses
  via the secrets socket
- `mc-mesh version`

The user-facing surfaces below all live in `integrations/mc/src/mesh.rs`
(where the existing `MeshCommand` enum already gathers `mc mesh up`,
`mc mesh agent`, `mc mesh task`, etc.). Phase 5c extends that enum.

```
mc mesh profile add <name> --url <…> --bootstrap-token <jt_…>
    Calls /runtime/nodes/register against the URL, stores the result
    plus URL+auth as a saved profile. Does NOT activate it.
    Replaces the temporary `mc-mesh node-register` subcommand from
    Phase 4b — that one moves into `mc mesh profile add`'s
    implementation and the mc-mesh binary loses the subcommand.

mc mesh profile list
    Lists saved profiles + which is active. Reads state.json.

mc mesh profile remove <name>
    Drops the profile entry. If active, prints "restart daemon to
    take effect" and clears active_profile.

mc mesh profile rename <old> <new>
    Cosmetic rename. Useful for the v1→v2 migration default ("default"
    → something meaningful).

mc mesh use <name>           # activate saved profile
mc mesh use --standalone     # deactivate (no controlplane)
...
mc mesh agent reassign <id> --mission <new>     # Phase 4d-b — same logic
mc mesh agent unenroll <id>                     # Phase 4d-b — same logic
mc mesh agent ls                                # query SQLite (both modes)
```
...
- **Direct file I/O** (when daemon may not be running): reads/writes
  `~/.mc/mc-mesh.state.json`, `~/.mc/mc-mesh.db`. Used by:
...
- **mc-mesh mgmt socket** (when daemon is running, for hints):
  `mc mesh agent enroll` etc. send a "reconcile" message to
  `~/.mc/mc-mesh-mgmt.sock` so the daemon re-runs its reconciler
...
```
$ mc-mesh use personal
Active profile: personal (was: work)
Restart the daemon to apply: systemctl --user restart mc-mesh
```

On daemon restart, it sees the new active_profile, registers under that
profile's identity (already done at `mc-mesh profile add` time), pulls
...
the TUI = same as `mc-mesh use <name>`.
...
- [ ] `mc-mesh-local-registry` module: open SQLite, run migrations,
      CRUD on `agent` table
- [ ] Daemon reads from SQLite when no active profile
- [ ] `mc-mesh agent enroll/reassign/unenroll/list` write to SQLite in
      standalone mode
...
### 5c — Profile management CLI (in `mc`, not `mc-mesh`)
...
- [ ] `MeshProfileCommand::{Add, List, Remove, Rename}` implementations
      that read/write `~/.mc/mc-mesh.state.json` directly.
- [ ] `MeshUseArgs` writes `active_profile`. Prints daemon-restart hint.
- [ ] Wire `mc mesh agent {enroll,reassign,unenroll,ls}` to branch on
      "active profile present?" — standalone path writes SQLite,
      federated path POSTs to controlplane.
- [ ] **Remove** the `node-register` subcommand from the `mc-mesh`
      binary added in Phase 4b. Its body moves into `mc mesh profile add`'s
      handler in `integrations/mc/src/`. Update `Phase 4b`'s state-file
      module so the mc binary can call it (move it from
      `crates/mc-mesh/src/state.rs` to a shared crate, e.g.
      `mc-mesh-core::state` or a new `mc-mesh-state` crate, so both `mc`
      and `mc-mesh` link against it).
...
| **4b** state file for node identity | becomes per-profile (schema v2). Migration from v1 is mechanical. The `mc-mesh node-register` subcommand introduced there is **removed** in Phase 5c — it was the wrong split and never should have lived on the daemon binary. Its logic moves into `mc mesh profile add`. |
...
1. **Migration default for old `state.json` v1 → v2.** The v1 file holds a
   ...
   rename via `mc-mesh profile rename default <better>`.
...
4. **Scope of secrets across profiles.** The mc-mesh-secrets broker
