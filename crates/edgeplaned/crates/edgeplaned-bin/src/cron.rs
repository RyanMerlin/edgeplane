//! edgeplaned cron tick loop + GC task — Phase 4 stage 4.
//!
...
impl CronLoop {
    ...
    /// with an empty config (operator can `edgeplane agent cron reload` later
            ...
            Err(e) => {
                tracing::warn!(
                    "cron: could not load {} on startup: {e:#}. \
                     ...
                     Fix the file and run `edgeplane agent cron reload`.",
                    config_path.display()
                );
                CronConfig {
                    schema_version: 1,
