...
//! the `edgeplane agent attach` surface in Phase 3.
//!
//! ## Task-mode
//!
    ...
    async fn attach_pty(&self, _handle: &AgentHandle) -> Result<PtySession> {
        bail!(
            "ZellijHostedRuntime does not support PTY attach; \
             use `edgeplane agent attach <id>` (exec zellij attach) or \
             `edgeplane agent attach --web <id>` (return zellij web URL) instead"
        )
    }
