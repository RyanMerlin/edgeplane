...
function Install-McIntegration {
    ...
    $mcpGithubSpec = if ($env:MCP_GITHUB_SPEC) { $env:MCP_GITHUB_SPEC } else { "git+https://github.com/edgeplane/edgeplane.git#subdirectory=distribution/edgeplane-mcp" }
    $docsUrl = if ($env:DOCS_URL) { $env:DOCS_URL } else { "https://github.com/edgeplane/edgeplane#readme" }
