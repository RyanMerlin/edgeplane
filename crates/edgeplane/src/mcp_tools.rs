use crate::{booster::AgentBooster, client::EdgeplaneClient, schema_pack::SchemaPack};
...
pub async fn fetch_tools_from_backend(client: &EdgeplaneClient) -> Result<Vec<Value>> {
    let response = client
        .get_json("/mcp/tools")
        .await
