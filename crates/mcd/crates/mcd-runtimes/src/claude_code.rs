...
use mcd_core::paths;
use mcd_core::progress::{ProgressEvent, ProgressEventType};
use mcd_core::types::{
...
impl ClaudeCodeRuntime {
    ...
    /// per-agent config. Extras are merged after the built-ins and deduped on
    /// the inner string so the matcher in `mcd-work::claim` sees a clean
    /// list. Order is preserved (built-ins first), which matters for any
    /// future capability-ordering semantics.
...
impl AgentRuntime for ClaudeCodeRuntime {
    ...
    async fn launch(&self, ctx: LaunchContext) -> Result<AgentHandle> {
        ...
        if output.is_err() {
            return Err(anyhow!(
                "claude CLI not found in PATH. Run `mc daemon runtime install claude-code`."
            ));
        }
    ...
    async fn inject_task(
        ...
        // Propagate the mcd capability socket path so agents can reach `mcd run`.
...
mod tests {
    use super::*;
    use mcd_core::progress::ProgressEventType;
