...
id 4  aria-mc-engineer    online   2026-05-11T11:50:05   fleet-management,mc-development,...
...
2. **Detect identity duplicates by node + capabilities** — flag rows likely to be the same agent (current snapshot would flag `merlinlabs` + `aria-merlinlabs`). Print a one-time migration report to the operator. Do **not** auto-merge — operator picks the canonical name via `mc admin agents merge --keep <id> --drop <id>`.
...
- `mc admin agents merge` CLI command for the migration cleanup.
- `mc admin agents reap --older-than 30d` for periodic cleanup.
