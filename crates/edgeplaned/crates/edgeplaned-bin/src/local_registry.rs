//! Local SQLite agent registry — `~/.ep/edgeplaned/registry.db` (mode 0600).
//!
...
impl LocalRegistry {
    /// Default DB path: `~/.ep/edgeplaned/registry.db`.
    pub fn default_path() -> Result<PathBuf> {
        Ok(edgeplaned_core::paths::registry_db_path())
    }
...
pub struct AgentLaunchContext {
    ...
    /// true, edgeplaned notices the unit is down but does NOT issue a restart.
    /// Operator-controlled via `edgeplane agent supervise pause/resume`.
...
impl LocalRegistry {
    ...
    /// List all rows in `agent_cron_state`. Used by `edgeplane agent cron list`
...
pub struct UnitRestartEntry {
    ...
    /// `'dead'` (unit was down) | `'nightly'` (hygiene restart) | `'manual'` (`edgeplane agent supervise restart`)
    pub reason: String,
