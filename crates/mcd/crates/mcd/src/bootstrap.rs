...
//!      `MC_HOME_DOMAIN_NAME` env var). Single global domain — *not* per-node.
...
/// `MC_HOME_DOMAIN_NAME` env var if a deployment wants a different name.
...
///
/// Priority:
///   1. `MC_HOME_DOMAIN_NAME` env var (deployment override).
///   2. `DEFAULT_HOME_DOMAIN_NAME` ("home").
pub fn resolve_home_domain_name() -> String {
    if let Ok(v) = std::env::var("MC_HOME_DOMAIN_NAME") {
        let trimmed = v.trim();
        if !trimmed.is_empty() {
...
mod tests {
    ...
    fn resolve_home_domain_name_env_override() {
        ...
        unsafe { std::env::set_var("MC_HOME_DOMAIN_NAME", "fleet-ops-test"); }
        let name = resolve_home_domain_name();
        unsafe { std::env::remove_var("MC_HOME_DOMAIN_NAME"); }
    ...
    fn resolve_home_domain_name_whitespace_falls_through() {
        unsafe { std::env::set_var("MC_HOME_DOMAIN_NAME", "   "); }
        let name = resolve_home_domain_name();
        unsafe { std::env::remove_var("MC_HOME_DOMAIN_NAME"); }
