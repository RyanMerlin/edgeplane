# mcd: Persistent Session Architecture
...
**Context:** Evaluated current mcd implementation against intended long-running agent session model.
...
mcd is the node daemon — analogous to RKE2/kubelet, not a job runner. It runs on every physical or virtual node, registers with mc-controlplane, and manages all agents on that node. Agents are long-running, persistent Claude sessions — not disposable one-shot processes.
...
- **Local PTY attach (deprecated by ACP-only decision)** — `attach_gateway` binds a Unix socket at `~/.missioncontrol/mcd.sock`. `mc daemon attach <agent-id>` connects locally, negotiates agent ID, then becomes a raw PTY proxy. This path is retained until ACP local attach lands; it does not factor into the persistent-session implementation plan below.
...
The `launch.sh` files in each Aria profile are doing this job today, outside mcd entirely. mcd needs to own this.
...
- mcd exposes a WebSocket endpoint per persistent agent that proxies the agent's ACP stream (JSON-RPC over stdio) bidirectionally
- mc-controlplane provides the proxy route: `GET /runtime/nodes/{node_id}/agents/{agent_id}/attach` → upgrades to WS → forwards JSON-RPC frames to the mcd node over Tailscale
- mcd nodes already maintain an outbound connection to controlplane for task loop; that channel is suitable as the back-channel, or mcd accepts inbound WS on a dedicated port (chosen at implementation time)
...
#### Gap 4 — No session lifecycle in mcd config
...
The persistent-session transport is ACP (Agent Client Protocol) end-to-end. mcd spawns `claude-code-acp` as the agent child process; messages flow as JSON-RPC over stdio inside the node, and as JSON-RPC over WebSocket between nodes / to clients.
...
- **Trivial relay.** Forwarding ACP between mcd ↔ controlplane ↔ client is a JSON-RPC frame pump. No SIGWINCH, no terminal geometry, no escape sequences.
...
mc-controlplane
  ...
  → upgrades to WS
  → forwards JSON-RPC frames to the mcd node
...
Back-channel between controlplane and mcd:
- **Option A**: mcd exposes a WS server on a local port; controlplane dials in when a client attaches. Simpler; relies on Tailscale for reachability.
- **Option B**: mcd opens a persistent multiplexed connection to controlplane at startup; attach streams are multiplexed over it. More complex but works even without direct reachability.

Tailscale is present on all nodes — Option A is the right call.

### mcd.yaml shape (when ready)

One config file per node at `~/.mc/mcd.yaml`. One daemon per node manages all agents on that node.
...
The `launch.sh` files are retired once mcd owns the session lifecycle.
...
**Files:**
- `mcd-runtimes/src/claude_code.rs` — change `new()` to `new(capabilities: Vec<Capability>)`
- `mcd/src/config.rs` — add `capabilities: Vec<String>` to `AgentEntry`
- `mcd/src/daemon.rs` — pass agent capabilities when constructing runtime

No conflict with any in-flight work. Can be done independently.

### Phase 1 — Persistent session runtime (mcd)
...
### Phase 2 — Remote ACP relay (controlplane + mcd)

- mcd: add WS server on a local port (e.g. `127.0.0.1:8009`) that accepts attach connections identified by `agent_id`. Each connection becomes a JSON-RPC frame pump between the caller and the agent's ACP client handle held by the session supervisor.
- mc-controlplane: add route `GET /runtime/nodes/{node_id}/agents/{agent_id}/attach` — resolves node Tailscale address, dials mcd WS, upgrades caller to WS, forwards JSON-RPC frames bidirectionally.
...
### Phase 4 — Dependency result injection in build_prompt() (mcd)
...
1. **Attach route design** — should controlplane proxy via Tailscale hostname (Option A) or multiplex over existing mcd→controlplane connection (Option B)?
