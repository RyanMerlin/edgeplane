...
helm upgrade --install edgeplane infra/helm/edgeplane \
  ...
  --set-string config.EP_TOKEN="${EP_TOKEN}"
```

Set `POSTGRES_PASSWORD` and `EP_TOKEN` from your secrets store (e.g., GitHub Actions secrets) before deploying. The GitHub workflow also uses those secrets when invoking the Helm upgrade. The chart relies on the Terraform outputs for the PostgreSQL host and any other runtime-specific endpoints.
