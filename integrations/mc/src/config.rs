...
pub fn load_server_list() -> Vec<String> {
    ...
    if let Ok(val) = env::var("MC_BASE_URL") {
        return vec![val];
    }
...
pub fn write_servers_file(servers: &[String]) -> std::io::Result<()> {
    ...
    let mut content = String::from("# MissionControl server endpoints\n");
    content.push_str("# Auto-generated by `mc discover` — edit only to override\n");
    for s in servers {
        content.push_str(s);
        content.push('\n');
    }
