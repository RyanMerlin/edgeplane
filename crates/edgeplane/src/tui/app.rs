...
/// require auth. This is the Phase 1 surface from `edgeplane-tui-auth-spec.md` —
...
pub enum AuthState {
    ...
    Anonymous,
    /// A valid saved session was loaded from `~/.ep/`.
    SessionValid {
        subject: String,
        email: Option<String>,
        expires_at: String,
...
pub struct App {
    ...
    /// fresh session (the user running `edgeplane auth login` in another shell).
...
impl App {
    ...
    /// TUI auto-recovers when the operator runs `edgeplane auth login` in another
                ...
                self.modal = Some(AppModal::Info {
                    modal: InfoModal {
                        ...
                        lines: vec![
                            ...
                            "".to_string(),
                            "Use `edgeplane auth logout` to clear the session.".to_string(),
                        ],
                    },
                });
            }
    ...
    fn refresh_doctor_snapshot(&mut self) {
        ...
        checks.push(DoctorCheckRow {
            ...
            hint: if matches!(cp_status, DoctorStatus::Err) {
                Some("Check EP_BASE_URL and that the controlplane is running.".to_string())
            } else {
                None
            },
        });
        ...
        checks.push(DoctorCheckRow {
            ...
            hint: if matches!(v_status, DoctorStatus::Warn) {
                Some("`edgeplane update` to align with the controlplane.".to_string())
            } else {
                None
            },
        });
        ...
        let (a_status, a_detail, a_hint) = match &self.auth_state {
            ...
            AuthState::SessionFromFlag => (
                DoctorStatus::Ok,
                "session token (OIDC via edgeplane auth login)".to_string(),
                None,
            ),
    ...
    fn auto_refresh(&mut self) {
        // Quietly notice if the operator ran `edgeplane auth login` in another
    ...
    pub fn handle_key(&mut self, key: KeyEvent) {
        ...
        // `edgeplane auth login` in another shell, this picks up the new session
...
fn error_to_check(name: &'static str, err: &Option<String>) -> DoctorCheckRow {
    match err {
        ...
        Some(msg) if msg.starts_with("Not signed in") => DoctorCheckRow {
            ...
            detail: msg.clone(),
            hint: Some(
                "Auto-recovers after `edgeplane auth login`; press R to retry now.".to_string(),
            ),
        },
