# edgeplaned Phase 6: Home Mission, Sync Loop, Goose-as-Router
...
Phase 5 made edgeplaned standalone-first with a local SQLite registry, and split `edgeplaned` (kubelet) from `edgeplane` (kubectl). What's missing:

1. **Bootstrap.** A freshly-registered node has no agents. The user has to hand-configure yaml or run `edgeplane daemon agent enroll` manually.
...
When a node registers (or, in standalone mode, on first `edgeplane daemon agent enroll-home`), the system provisions:
...
edgeplaned already supports running N task_loops concurrently — Phase 5 just didn't make use of it dynamically. Phase 6 codifies the shape:
...
|---|---|---|
| Home mission auto-provision | `edgeplane daemon agent enroll-home` (manual, one-time) | `edgeplane daemon profile add` triggers backend `provision_home_for_node` (automatic) |
| Mission/agent CRUD | Local-only: `edgeplane daemon mission create / agent enroll` writes to SQLite via `mgmt_gateway` | Controlplane API; daemon syncs back to SQLite |
...
## edgeplane CLI changes

```
edgeplane daemon profile add ... --no-home          # opt out of home-mission auto-provision
edgeplane daemon agent enroll-home                  # standalone-mode: create home mission locally
edgeplane daemon use <profile>                      # already exists — re-sync after switch
edgeplane daemon status                             # extend to show: home mission, active assignments, sync-loop state
```

`edgeplane daemon status` is also where we fix the existing "daemon: stopped" detection bug — it should check the mgmt socket (`~/.ep/edgeplaned-mgmt.sock`) and return its agent/mission view, not just look at a PID file.

---

## Migration from yaml

For nodes that currently boot from `~/.ep/edgeplaned.yaml`:

1. yaml continues to work for at least one release — it loads into LocalRegistry under `SOURCE_LOCAL`
2. When a profile is added with `edgeplane daemon profile add`, the sync loop activates and pulls authoritative state from the controlplane
3. yaml is treated as additive but lower-precedence than controlplane-sourced agents
4. New deprecation warning: "edgeplaned.yaml is legacy. Run `edgeplane daemon profile add` to federate, or `edgeplane daemon agent enroll-home` to use standalone discovery."
...
- **In-task polling** (`edgeplane daemon messages` from inside an agent workdir). Phase 5d's pending-message buffer covers the bulk of the coordination need; in-task polling waits for a concrete mid-task-pivot use case.
...
- **Goose MCP tooling for `edgeplane.*`** so the home-Goose can call `create_task` / `detect_overlaps` itself. Hooking Goose to MCP is a separate work item — for v0 the home agent's actions go through `edgeplane` CLI invocations.
...
4. ✅ **edgeplaned sync_loop** — *already shipped* in Phase 4d/5d. `resolve_agent_specs` does the initial controlplane pull, `Spawner::apply_plan` handles spawn/restart/shutdown, `watch_assignments_ws` + `poll_assignments` keep state converged. No new code needed; verified end-to-end wiring.
5. ✅ **edgeplane CLI: `enroll-home` + extended `status`** — `7d1813f`. `edgeplane daemon agent enroll-home` for standalone-mode home-mission setup. Status now shows mode/profile/agents and uses mgmt-socket probe so it correctly reports `running` for the systemd-managed daemon.
6. ✅ **edgeplane CLI: `profile add` Tailscale auto-detection** — `edc0169`. Reads `tailscale status --json` when `--tailscale-fqdn` not given, surfaces home-mission info from the register response.
...
A clean `edgeplane daemon profile add ...` on a fresh node now yields a fully-bootstrapped node with a home-Goose agent ready to receive routing tasks.
...
2. **Goose binary distribution.** edgeplaned-runtimes already has a `goose` runtime. Confirm the binary is installable on a fresh node (or have `ensure_installed` handle it) so home-mission Goose provisioning doesn't fail on bare metal.
