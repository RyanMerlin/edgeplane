...
pub enum RuntimeKind {
    ...
    /// the named session — there is no PTY owned by edgeplaned. The per-agent
...
pub enum StateDirSpec {
    ...
    Ephemeral {
        ...
        /// (`edgeplaned.reaper.default_ttl_minutes`, 60).
...
pub struct LaunchContext {
    ...
    /// Base URL of the Edgeplane backend.
...
pub enum SupervisorEvent {
    ...
    /// edgeplaned issued `systemctl --user restart`. `result` is "started"
    ...
    /// `edgeplane agent supervise pause`. Auto-restart is suspended until
    /// they `resume`.
    SupervisePaused {
        agent_id: String,
        source: String,
