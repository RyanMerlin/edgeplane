...
use anyhow::Result;
use edgeplaned_core::types::{Capability, PtySession, TaskSpec};
use std::path::Path;
...
/// Returns the directory to prepend to PATH so agents can invoke `edgeplane`.
///
/// Resolution order:
...
mod tests {
    ...
    fn build_prompt_renders_dependency_results_between_roster_and_task() {
        use edgeplaned_core::types::{DependencyResult, TaskSpec};
        let task = TaskSpec {
            id: "t-2".into(),
            mission_id: "k-1".into(),
            domain_id: "m-1".into(),
    ...
    fn build_prompt_omits_dependency_section_when_empty() {
        use edgeplaned_core::types::TaskSpec;
        let task = TaskSpec {
            id: "t-1".into(),
            mission_id: "k-1".into(),
            domain_id: "m-1".into(),
    ...
    fn build_prompt_renders_pending_messages_before_task() {
        use edgeplaned_core::types::{PendingPeerMessage, TaskSpec};
        let task = TaskSpec {
            id: "t-3".into(),
            mission_id: "k-1".into(),
            domain_id: "m-1".into(),
