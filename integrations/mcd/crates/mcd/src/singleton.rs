//! Kernel-enforced singleton via flock(2) on the mcd lock file.
//!
//! ## Why flock
//!
...
//! mcd holds the lock, this function returns a structured error that an
    ...
    pub fn acquire(lock_path: &Path, kill_existing: bool) -> Result<Self> {
        ...
        match Self::try_lock(lock_path) {
            ...
            Err(LockError::WouldBlock) if kill_existing => {
                ...
                eprintln!("--kill-existing: terminating mcd PID {pid}");
                kill_holder(pid)?;
                ...
                match Self::try_lock(lock_path) {
                    ...
                    Err(LockError::WouldBlock) => Err(anyhow!(
                        "lock at {lock_path:?} still held after --kill-existing \
                         terminated PID {pid}; another mcd may have started in the \
                         meantime. Run `mcd doctor` to inspect."
                    )),
...
fn format_holder_error(lock_path: &Path) -> String {
    ...
    format!(
        "another mcd is already running.\n\n  \
         holder PID:     {pid}\n  \
         ...
         1. If the holder is healthy:\n       \
              systemctl --user restart mcd.service     # systemd-managed\n       \
              kill -TERM {pid}                          # shell-launched\n  \
         2. If hung or unresponsive:\n       \
              mcd run --kill-existing                   # SIGTERM, then SIGKILL, then take over\n  \
         3. To inspect before deciding:\n       \
              ps -p {pid} -o pid,etime,cmd\n       \
              journalctl --user -u mcd.service -n 50\n       \
              mcd doctor",
        lock_path.display(),
    )
}
...
mod tests {
    ...
    fn acquire_creates_lock_file_and_writes_identity() {
        ...
        let path = dir.path().join("mcd.lock");
    ...
    fn second_acquire_in_same_process_fails_with_holder_info() {
        ...
        let msg = format!("{err}");
        assert!(msg.contains("another mcd is already running"), "msg: {msg}");
        assert!(msg.contains(&format!("holder PID:     {}", std::process::id())));
        assert!(msg.contains("mcd run --kill-existing"));
    }
    ...
    fn read_holder_parses_kv_lines() {
        ...
        assert_eq!(info.binary.as_deref(), Some("/usr/bin/mcd"));
