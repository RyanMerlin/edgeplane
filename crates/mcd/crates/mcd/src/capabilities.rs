...
//! | `mc:read`    | mc agent/daemon read commands               |
//! | `mc:write`   | mc:read + submit/enroll/signal              |
...
fn capability_vocabulary() -> HashMap<&'static str, Vec<&'static str>> {
    ...
    // mc:read — MC CLI status/inspect commands (no state-mutation).
    m.insert(
        "mc:read",
        vec![
            "Bash(mc agent ls *)",
            "Bash(mc daemon agent ls *)",
            "Bash(mc daemon task ls *)",
            "Bash(mc agent cron list)",
            "Bash(mc agent cron describe *)",
            "Bash(mc status *)",
        ],
    );

    // mc:write — subsumes mc:read plus submission / enrollment / signalling.
    m.insert(
        "mc:write",
        vec![
            // Inherited from mc:read
            "Bash(mc agent ls *)",
            "Bash(mc daemon agent ls *)",
            "Bash(mc daemon task ls *)",
            "Bash(mc agent cron list)",
            ...
            "Bash(mc status *)",
            // Write operations
            "Bash(mc daemon task submit *)",
            "Bash(mc daemon agent enroll *)",
            "Bash(mc agent signal *)",
        ],
    );
...
mod tests {
    ...
    fn translate_mc_write_includes_mc_read_fragments() {
        let tools = translate_capabilities(&["mc:write".to_string()]).unwrap();
        let cli = tools.to_cli_string();
        // mc:read fragment that should be subsumed
        assert!(cli.contains("Bash(mc agent ls *)"), "missing mc:read fragment in: {cli}");
        // mc:write-specific fragment
        assert!(cli.contains("Bash(mc agent signal *)"), "missing mc:write fragment in: {cli}");
    }
