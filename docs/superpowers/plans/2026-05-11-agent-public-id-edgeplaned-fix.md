# Agent public_id + edgeplaned Delivery Fix

**Date:** 2026-05-11  
**Author:** aria-merlinlabs  
**Priority:** High — blocks all edgeplaned message delivery to Claude Code agents
...
### Part 1 — edgeplane-tower: Add `public_id`

**1.1 Migration** — `crates/edgeplane-tower/migrations/0008_agent_public_id.sql`
...
Include `public_id` in all agent responses so edgeplaned can record it after enrollment.

---

### Part 2 — edgeplaned: Strip `/work/` + use public_id
...
`edgeplaned/src/daemon.rs` — `agent_spec_from_json()` (lines 827-881):
...
`edgeplaned/src/local_registry.rs` (line 59): update upsert to store `public_id` from controlplane response, not the local UUID.

---

### Part 3 — edgeplane CLI: Display + routing

**3.1** — `edgeplane daemon agent ls`: show `public_id` as primary identifier, drop numeric `id` from default output  
**3.2** — `edgeplane agent remote message --agent-id` / `--to-agent-id`: already accepts strings; works once route handler accepts public_id  
...
```
Step 1: edgeplane-tower — migration + public_id generation (foundation)
Step 2: edgeplane-tower — AgentIdent extractor + route updates
Step 3: edgeplaned — client prefix fix (unblocks ALL /work/ routes)
Step 4: edgeplaned — enrollment reads public_id, stores it, uses for polls
Step 5: edgeplane CLI + dashboard — display update (can trail, cosmetic)
```
...
**Suggested branch:** `feat/agent-public-id` covering Steps 1-4 together — they need to ship atomically since edgeplaned enrollment must read a public_id that the controlplane generates.
...
```
edgeplane agent remote message --agent-id aria-merlinlabs-x1y2z3a4 --to-agent-id aria-work-qwn5eb33 --content "wake up"
  → controlplane resolves public_id → stores in inbox
  → edgeplaned polls /agents/aria-work-qwn5eb33/messages (no /work/ prefix, correct ID)
  → edgeplaned injects into Claude Code session via ACP
...
| File | Change |
|------|--------|
| `crates/edgeplane-tower/migrations/0008_agent_public_id.sql` | New migration |
| `crates/edgeplane-tower/src/models/agent.rs` | Add public_id field |
| `crates/edgeplane-tower/src/routes/agents.rs` | AgentIdent extractor, generate on insert |
| `crates/edgeplaned/crates/edgeplaned-core/src/client.rs` | Strip api_prefix |
| `crates/edgeplaned/crates/edgeplaned/src/daemon.rs` | Read public_id from enrollment |
| `crates/edgeplaned/crates/edgeplaned/src/local_registry.rs` | Store public_id |
