...
impl ClaudeCodeRuntime {
    ...
    /// the inner string so the matcher in `edgeplaned-work::claim` sees a clean
...
impl AgentRuntime for ClaudeCodeRuntime {
    ...
    async fn launch(&self, ctx: LaunchContext) -> Result<AgentHandle> {
        ...
        if output.is_err() {
            return Err(anyhow!(
                "claude CLI not found in PATH. Run `edgeplane daemon runtime install claude-code`."
            ));
        }
...
mod tests {
    use super::*;
    use edgeplaned_core::progress::ProgressEventType;
