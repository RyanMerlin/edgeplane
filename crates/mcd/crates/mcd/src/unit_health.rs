...
//! agent isn't paused, mcd issues `systemctl --user restart <service>`
...
/// via [`UnitHealthConfig`] in mcd config.
...
struct AgentState {
    /// When mcd last issued a restart for this agent. Throttle window
    /// reads from this.
...
impl UnitHealthLoop {
    ...
    async fn process_tick(&mut self) {
        ...
        for ctx in agents {
            ...
            // Skip mcd itself if someone accidentally configures it —
            // restarting your own supervisor is a recipe for chaos.
            if service.starts_with("mcd") {
                continue;
            }
...
/// `mc agent supervise list`. Returned as a flat list of
