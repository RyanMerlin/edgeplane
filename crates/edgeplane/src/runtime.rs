...
async fn run_node_run(args: NodeAgentRunArgs, client: &EdgeplaneClient) -> Result<()> {
    // ── DEPRECATION NOTICE ────────────────────────────────────────────────────
    // `edgeplane node run` is the legacy node-agent path.  It will be removed once
    // `edgeplane daemon` reaches full feature parity.  The recommended path is:
    //
    //   edgeplane daemon up                          # start the edgeplaned daemon
    ...
    eprintln!(
        "⚠  edgeplane node run is deprecated. Use `edgeplane daemon up` instead.\n\
         See `edgeplane daemon --help` for the new work-model commands.\n"
    );
...
fn build_goose_command(
    ...
    if trimmed.starts_with('/') || trimmed.ends_with(".yaml") || trimmed.ends_with(".json") {
        ...
        let tmp = std::env::temp_dir().join(format!("edgeplane-goose-{}.json", Uuid::new_v4()));
