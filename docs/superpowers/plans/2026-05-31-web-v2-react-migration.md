...
- [x] **6.4** Retire Svelte — **DONE 2026-06-04.** `git rm -r web/` (deleted the dead SvelteKit app) + `git mv web2 web` (React app now at the canonical `web/`). Updated all path refs: Dockerfile web-builder stage, `web.yml` CI, `gen_openapi.rs` doc comment, `README.md`, and the npm package name (`web2` → `edgeplane-web`).
- [x] **6.5 Rollback:** Now via git history — `web/` (SvelteKit) is no longer retained in-tree. To restore: `git checkout <pre-cutover-sha> -- web/` then revert the Dockerfile stage. The Dockerfile-only revert path no longer applies.

## Phase 7 — Federated zellij attach (code FIXED on PR #10; deploy pending)

Make a dashboard tab open a **live ACP conversation**. Today tabs render the tower's registered agents but are **monitoring-only** — attach is not wired. This is an `edgeplaned` daemon feature, tracked on **PR #10** `feat/edgeplaned-federated-launch-overrides` (the blocking gap is now closed — see below; the live-fleet deploy is the only remaining step). Design + safety are done — 4 impl rounds + 3 adversarial reviews; the federated reconcile is a verified **no-op for live sessions** and standalone is byte-identical (so deploying the daemon is safe), and signal-by-name is preserved via a supervisor alias.

**Blocker — FIXED 2026-06-04 (commit `860e1a5`, rebased on main, reviewed).** The launch-override merge keyed solely on the controlplane `name` field, but `GET /runtime/nodes/{id}/agents` returns `name=null` — the profile is the **middle** segment of the public_id, confirmed against the live fleet as `aria-<profile>-<hex>` (e.g. `aria-research-22e6cd17`), not a `-{profile}` suffix. The unit fixtures used `name="aria-engineer"`, which masked it. `merge_federated_overrides` now falls back to a new pure `profile_from_public_id` helper (middle segment between the first and last `-`, hyphenated profiles preserved) when `name` is absent. Adversarial review (`rust-reviewer`) caught a latent false-positive — a hyphenated derived profile (`foo-bar`) would suffix-match a lone `bar` context — so the suffix clause is now gated to name-based keys only; exact-match for derived keys. Real-shape integration tests added (incl. end-to-end through `agent_spec_from_json` with `name: null`, the gated no-false-match case, and name>public_id precedence); **24/24 daemon tests pass**, `daemon.rs` clippy-clean.

**Remaining — the deploy (NOT yet done; do in a focused session with real integration testing):** build/hot-swap `edgeplaned`, register the node, enroll the 6 profiles as `<profile>`, set each agent's `metadata.node_id`, restart federated, verify attach end-to-end (click a tab → live conversation). Pre-existing toolchain debt to watch: 3 `collapsible_if` clippy errors in the separate `edgeplaned-receipts` crate (rust 1.95 bump) may fail a clippy-gating CI — trivial separate cleanup.
